	...
	*/
	...
	public function index()
	{
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="1")
		{
			...
			$this->load->view('dashboard/administrator/master/v_perusahaan',$d);
		}
		else
		{
	...
	function simpan_perusahaan(){
		...
		$data=$this->m_perusahaan->simpan_perusahaan($gol,$es,$ga);
	...
	function get_perusahaan(){
		...
		$data=$this->m_perusahaan->get_perusahaan_by_kode($kobar);
		echo json_encode($data);
		}
		else
		{
	...
	function get_perusahaan_json() { 
		if($this->session->userdata('logged_in')!="" && $this->session->userdata('stts')=="1")
		{
	    header('Content-Type: application/json');
	    echo $this->m_perusahaan->perusahaan_list();
		}
		else
		{
			header('location:'.base_url().'');
	...
	function update_perusahaan(){
		...
		$data=$this->m_perusahaan->update_perusahaan($a2,$b2,$c2,$d2,$kode);
		echo json_encode($data);
		}
		else
		{
